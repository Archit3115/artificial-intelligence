...
Typical flow
Client requests a LiveKit token from the Application Server.

Client connects to LiveKit SFU using the token (WSS) and establishes a WebRTC PeerConnection.

Client publishes local microphone/audio tracks to the SFU.

Backend spawns or signals an AI Agent to join the same room.

AI Agent subscribes to the user's audio, processes/transcribes it, generates a response, and sends text to ElevenLabs.

ElevenLabs returns synthesized audio, which the AI Agent publishes back to the LiveKit room.

Clients subscribed to the AI Agent hear the generated audio in real time.
...
graph TD
    ...
    E -- "HTTP/WebSocket Request" --> J
    J -- "Returns Synthesised Audio" --> E

    style A fill:#cde4ff,stroke:#333
    style SDK fill:#cde4ff,stroke:#333
...
Typical flow
Client requests a LiveKit token from Application Server.
